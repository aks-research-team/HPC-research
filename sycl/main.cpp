...
int main() {
    // Set constants
    constexpr size_t total_time = 10000;
    constexpr size_t log_freq = 10;
    constexpr size_t save_freq = 500000000;
    ...
    for (size_t t = 0; t < total_time; t++) {
        ...
        if ((t + 1) % save_freq == 0) {
            ...
            csv_file.open("oneapi/data_" + std::to_string(t) + ".csv");
            csv_file << "x,y,z,vx,vy,vz,p\n"; 
            auto e_acc = buffers_4d[0][4].get_access<dp_read>();
            auto vx_acc = buffers_4d[0][1].get_access<dp_read>();
            auto vy_acc = buffers_4d[0][2].get_access<dp_read>();
            auto vz_acc = buffers_4d[0][3].get_access<dp_read>();

            for (size_t i = 0; i < n-3; i++) {
                for (size_t ii = 0; ii < n-3; ii++) {
                    for (size_t iii = 0; iii < n-3; iii++) {
                        csv_file << i << ',' << ii << ',' << iii << ','
                            << vx_acc[i+2][ii+3][iii+3] << ',' << vy_acc[i+3][ii+2][iii+3] << ',' << vz_acc[i+3][ii+3][iii+2] << ','
                            << e_acc[i+3][ii+3][iii+3] << '\n';
                    }
                }
            }
            csv_file << '\n';
